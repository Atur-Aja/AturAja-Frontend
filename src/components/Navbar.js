...
import { IconMenu, IconToday, IconClose} from "./Icons";
...
export default function Navbar({ toggle, toggleCreate, toggleToday, isLanding }) {
  ...
  return (
    <div className="relative md:flex md:justify-between px-4 py-2 md:px-2 md:py-4 bg-biruTua sticky top-0 z-50 shadow-xl md:space-x-10 md:h-full">
      ...
      {isLanding ? (
        <div className={`${isMobile ? 'block' : 'hidden'} md:flex flex-col md:flex-row content-center text-white font-mulish`}>
          ...
          <div className="flex md:flex-row content-center justify-between py-2">
            <Link to="/login">
              <button className="bg-biru hover:bg-biruTua px-4 md:px-8 py-2 rounded md:rounded-full ml-0 md:ml-2 md:ml-4 mr-2 shadow-2xl">Login</button>
            </Link>
            <Link to="/signup">
              <button className="bg-white hover:bg-biruTua hover:text-white px-4 md:px-8 py-2 rounded md:rounded-full text-gray-600 ml-2 shadow-2xl">Register</button>
            </Link>
          </div>
        </div>
      ) : (
